...
public class T1 {
    ...
    // 所有测试用例将输出到此单一目录
    String outputDir = "D:\\Project\\github\\test\\generator-test-sp3\\src\\main\\java";
    ...
    public void testModelCustomization() {
        GeneratorHelper.mybatisPlusGenerator(url, username, password)
                ...
                .controller(c -> c.superClass("com.example.custom.BaseController")
                        .restStyle(true) // 使用 @GetMapping 等
     ...
     */
    @Test
    public void testSqlBoosterIntegration() {
        MybatisPlusSqlBoosterBuilder builder = GeneratorHelper.mybatisPlusSqlBoosterGenerator(url, username, password);
        builder.global(g -> g.outputDir(outputDir).fileOverride(true))
                .strategy(s -> s.include("sys_user"))
                .execute();
    }
}
